...
export const Collection: React.FC<CollectionProps> = ({ players, currentPlayerId }) => {
  // Defensive check: If players array is missing or empty, render nothing safely
  ...
  return (
    <div className="w-full max-w-6xl mx-auto mb-2 px-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {players.map((player) => {
          // CRITICAL SAFETY CHECK: If a player object is undefined (sync gap), skip rendering it
          if (!player) return null;

          // Safe access to properties with fallbacks
          const handLength = player.hand ? player.hand.length : 0;
          const score = player.collection 
            ? Object.values(player.collection).reduce((a: number, b: any) => (a||0) + (b||0), 0) 
            : 0;
          ...
          return (
            ...
            >
              <div className="flex justify-between items-center mb-4 border-b border-black/5 pb-2">
                  <div className="flex items-center gap-3">
                      <span className="font-black text-stone-700 text-sm uppercase tracking-wide flex items-center gap-2">
                          {player.name || 'Unknown Player'} {player.isAi ? '🤖' : '👤'}
                      </span>
                      {/* Hand Count Badge */}
              ...
              <div className="flex flex-wrap gap-3 min-h-[64px] items-center">
                  {player.collection && Object.entries(player.collection).map(([type, count]) => {
                  ...
                  {(!player.collection || Object.keys(player.collection).length === 0) && (
